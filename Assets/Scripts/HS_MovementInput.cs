...
public class HS_MovementInput : MonoBehaviour
{
    ...
    void PlayerMoveAndRotation()
    {
        ...
        if (blockRotationPlayer == false)
        {
            ...
            //Limit back speed
            if (vertical < -0.5)
                controller.Move(desiredMoveDirection * Time.deltaTime * (speed / 1.5f));
    ...
    void InputMagnitude()
    {
        ...
        anim.SetFloat("Vertical", vertical, VerticalAnimTime, Time.deltaTime * 2f);
        anim.SetFloat("Horizontal", horizontal, HorizontalAnimSmoothTime, Time.deltaTime * 2f);
